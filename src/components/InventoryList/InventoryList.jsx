...
export default function InventoryList({ setOpenModal}) {
    ...
    useEffect(() => {
        ...
        .then((r) => {
            setInventoryList(r.data)
        })
        .catch((e)=>{console.log(e)})
    }, [])

    let inventoryItems = inventoryList.map((item) => {
        return (
            <li className='warehouse__item'>
                    ...
                    <div className='warehouse__left'>
                        <div className='warehouse__left-name'> 
                            ...
                            <button className='warehouse__left-name-button'>
                                <Link to={`/inventory/${item.id}`} className='no_underline'><h3 className='warehouse__text-blue no_underline'>{item.item_name}</h3></Link>
                                <img src={chevron} className='grow no_underline' alt="chevron arrow right" />
                            </button>
                        </div>
                ...
                <div className='warehouse__icons'>
                    <button id='delete__button' className='warehouse__icons-delete grow'><img src={trash} alt="delete trash can" onClick={() => { setOpenModal(true) }}/></button>
                    <button id='edit__button' className='warehouse__icons-edit grow'><Link to={`/edit-inventory-item/${item.id}`}><img src={edit} alt="edit pencil" /></Link></button>
                </div>
            </li>
        )
    })
    ...
    return(
        <div className='warehouses'>
            <div className='warehouses__header'>
                ...
                <button className='warehouses__button'> <Link to="/add-new-inventory-item" className='no_underline warehouses__button'> <h3>+ Add New Item</h3> </Link></button>
            </div>
